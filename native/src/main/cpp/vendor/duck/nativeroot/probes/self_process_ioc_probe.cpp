...
namespace duckdetector::nativeroot {
    ...
    ProbeResult run_self_process_ioc_probe() {
        ...
        // RN fork: inaccessible proc data is reduced coverage, never a clean check.
        result.checked_count = (!context.empty() ? 1 : 0) + (proc_fd >= 0 ? 1 : 0);
        result.denied_count = 2 - result.checked_count;
